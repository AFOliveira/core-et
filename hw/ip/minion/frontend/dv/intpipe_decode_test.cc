...
DecodeFields capture(SimCtrl<DUT>& sim, uint32_t inst) {
    ...
    return DecodeFields{
        ...
        static_cast<uint8_t>(sim.dut->dec_alu_fn_o),
        static_cast<uint8_t>(sim.dut->dec_sel_alu1_o),
        static_cast<uint8_t>(sim.dut->dec_sel_alu2_o),
        static_cast<uint8_t>(sim.dut->dec_sel_imm_o),
        static_cast<bool>(sim.dut->dec_rxs1_o),
        static_cast<bool>(sim.dut->dec_rxs2_o),
        static_cast<bool>(sim.dut->dec_wxd_o),
    };
}
...
int main(int argc, char** argv) {
    ...
    // Strict ISA migration: NEW custom-1 vs encoding_plan reference fields;
    // OLD OP-32 words must not remain live in the migrated decoder.
    check_rtype_plan_fields(sim, 0x80C5E52B, kAluPackb, "packb_new_runtime");
    check_old_encoding_retired(sim, 0x80C5E52B, 0x80C5E53B, "packb_old_retired");

    check_rtype_plan_fields(sim, 0x80C5F52B, kAluBitmixb, "bitmixb_new_runtime");
    check_old_encoding_retired(sim, 0x80C5F52B, 0x80C5F53B, "bitmixb_old_retired");

    check_ctrl_equivalence(
        sim, 0x0075C52B, 0x00758513, int_alu, "aif.europeriscvsummit_vs_addi");
    check_must_differ(
        sim, 0x0075D52B, 0x00758513, "aif.europeriscvsummit_bad_funct3");

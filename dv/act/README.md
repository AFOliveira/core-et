...
with `VpuEn=0`. Floating-point, privileged/timer/interrupt, and `Zifencei`
self-modifying-code tests are left out until those platform contracts are
exposed and passing through the same runner.
...
flip-flop based and stable under full-core Verilator timing. It also uses
ACT-only RF preview models so Verilator sees architectural write data directly
at the RF write edge; the production RTL source is not changed.
